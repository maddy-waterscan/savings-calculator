(function(){
  ...
  const WS_DEFAULT_CONFIG = {
    ...
    caps: {
      grossSavingPct: 0.60,   // 60%
      barSavePct: 0.80        // 80% visual cap
    },
    inputs: {
      sites:   { min: 1, max: 200, step: 1,   value: 5,    label: 'Number of sites' },
      use:     { min: 100, max: 500000, step: 100, value: 20000, label: 'Average annual water use per site (m³)' },
      tariff:  { min: 0.1, max: 10, step: 0.05, value: 2.20, label: 'Current water cost (£ per m³)' },
      eff:     { min: 0, max: 30, step: 1,   value: 8,    label: 'Expected efficiency gain (%)' },
      leak:    { min: 0, max: 30, step: 1,   value: 5,    label: 'Leak loss reduction (%)' },
      service: { min: 0, max: 500000, step: 100, value: 25000, label: 'Annual Waterscan service fee (£)' },
      co2:     { min: 0, max: 2, step: 0.01, value: 0.34, label: 'Carbon factor (kg CO₂e per m³)' }
    },
    chart: {
  ...
  function applyConfig(){
    // Labels
    const labelMap = {
      wsSites: 'sites', wsUse: 'use', wsTariff: 'tariff',
      wsEff: 'eff', wsLeak: 'leak', wsService: 'service', wsCO2: 'co2'
    };
    Object.entries(labelMap).forEach(([id, key]) => {
      const labelEl = document.querySelector(`label[for='${id}']`);
      if (labelEl && CFG.inputs[key]?.label) labelEl.textContent = CFG.inputs[key].label;
    });

    // Texts
    const title = qs('.ws-title'); if (title) title.textContent = CFG.texts.title;
    const sub = qs('.ws-sub'); if (sub) sub.textContent = CFG.texts.sub;
    const cta = el('wsCTA'); if (cta) cta.textContent = CFG.texts.cta;
    const note = qs('.ws-note'); if (note) note.textContent = CFG.texts.note;
    const legBase = qs('.ws-leg-base'); if (legBase) legBase.textContent = CFG.texts.legBase;
    const legSave = qs('.ws-leg-save'); if (legSave) legSave.textContent = CFG.texts.legSave;
    qs('#wsWaterSaved')?.nextElementSibling && (qs('#wsWaterSaved').nextElementSibling.textContent = CFG.texts.kpiWater);
    qs('#wsMoneySaved')?.nextElementSibling && (qs('#wsMoneySaved').nextElementSibling.textContent = CFG.texts.kpiMoney);
    qs('#wsNetSavings')?.nextElementSibling && (qs('#wsNetSavings').nextElementSibling.textContent = CFG.texts.kpiNet);
    qs('#wsCO2Saved')?.nextElementSibling && (qs('#wsCO2Saved').nextElementSibling.textContent = CFG.texts.kpiCO2);
    ...
    function setAttrs(id, meta){
      ...
      ['min','max','step','value'].forEach(k => {
        if (meta[k] !== undefined) { range[k] = meta[k]; num[k] = meta[k]; }
      });
    }
  ...
  function bindPair(rangeEl, numEl){
    rangeEl.addEventListener('input', ()=>{ numEl.value = rangeEl.value; calc(); });
    numEl.addEventListener('input', ()=>{ rangeEl.value = numEl.value; calc(); });
  }

  function fmtGBP(x){
    return new Intl.NumberFormat(I18N.locale,{style:'currency',currency:I18N.currency,maximumFractionDigits:0}).format(x||0);
  }
  function fmtNum(x){
  ...
  function calc(){
    ...
    const grossPct = Math.min(CFG.caps.grossSavingPct, Math.max(0, (effP + leakP)/100));
    ...
    const savePct = baseSpend > 0 ? Math.min(CFG.caps.barSavePct * 100, (netPos / baseSpend) * 100) : 0;
    barSave.style.width = savePct + '%';
    barBase.style.width = (100 - savePct) + '%';

    const payload = { nSites, usePerSite, price, effP, leakP, fee, cf, waterSaved, moneySaved, netSavings, baseSpend };
    document.getElementById('waterscan-calculator').dispatchEvent(new CustomEvent('ws:update', { detail: payload }));
  }
  ...
  [ [sites,sitesNum], [use,useNum], [tariff,tariffNum], [eff,effNum], [leak,leakNum], [service,serviceNum], [co2,co2Num] ].forEach(([a,b])=>bindPair(a,b));
  ...
  document.getElementById('wsCTA').addEventListener('click', ()=>{
    ...
    document.getElementById('waterscan-calculator').dispatchEvent(new CustomEvent('ws:cta', { detail: data }));
